...
def dataset_quality_report(df):
    ...
    empty_columns = int(
        df.columns[df.isnull().all()].shape[0]
    )
    ...
    # Health Status
    st.write("### Dataset Health Status")
    ...
    try:
        ...
        with tab4:
            ...
            for col in numeric_cols:
                ...
                outliers = display_df[
                    (display_df[col] < lower) |
                    (display_df[col] > upper)
                ]
            ...
            health_score = 100
            health_score -= min(
                int(total_missing / max(len(display_df), 1) * 100),
                40
            )
...
""")
                        ...
                        if st.button(
                            ...
                            auto_cleaned_df, cleaning_log, warning_log, outlier_cols = (
                                auto_clean_dataset(working_df)
                            )
                            ...
                            after_missing = (
                                auto_cleaned_df.isnull().sum().sum()
                            )
                ...
                with cleaning_tab2:
                    ...
                    cleaned_df = data_cleaning_module(
                        st.session_state.working_df
                    )
            ...
            with st.expander(
                ...
                if (
                    "cleaning_completed" in st.session_state and
                    st.session_state.cleaning_completed
                ):
